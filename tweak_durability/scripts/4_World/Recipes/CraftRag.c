modded class CraftRag extends RecipeBase	
{
	...
	// knives take 0.5 damage instead of 10 when cutting up clothing (why would a knife even take damage from clothing tho?)
	//
	// fun fact (in vanilla) -> melee weapons take 0.5 damage from opening a can but take 10 damage from cutting up clothing
	...
	override void Init()
	{
		...
		// ˇknives take 0.5 damage instead of 10 when cutting up clothing
		
		m_IngredientAddHealth[0] = -0.5;// 0 = do nothing
		
		// ^knives take 0.5 damage instead of 10 when cutting up clothing
