	...
	override protected void SetNVMode(int mode)
	{
		...
		switch (mode)
		{
			...
			case NV_DEFAULT_OPTICS: //NV optic on
				SetTargetValueColor(PostProcessEffectType.Glow,PPEGlow.PARAM_COLORIZATIONCOLOR,{0.5,1,0.5,0},PPEGlow.L_23_NVG,PPOperators.MULTIPLICATIVE);
			...
			case NV_DAYTIME_OPTICS: //NV optic in daytime mode
				SetTargetValueColor(PostProcessEffectType.Glow,PPEGlow.PARAM_COLORIZATIONCOLOR,{0.5,1,0.5,0},PPEGlow.L_23_NVG,PPOperators.MULTIPLICATIVE);
				SetTargetValueFloat(PPEExceptions.EXPOSURE,PPEExposureNative.PARAM_INTENSITY,false,0.5 * m_UGExposureCoef,PPEExposureNative.L_0_NVG_OPTIC,PPOperators.ADD);
